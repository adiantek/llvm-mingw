...
if [ -z "$CHECKOUT_ONLY" ]; then
    if [ -z "$PREFIX" ]; then
        echo $0 [--enable-asserts] [--stage2] [--thinlto] [--lto] [--full-llvm] [--with-python] [--host=triple] dest
        exit 1
    fi
...
if [ -n "$HOST" ]; then
    ...
    if [ -n "$WITH_PYTHON" ]; then
        ...
        CMAKEFLAGS="$CMAKEFLAGS -DPython3_LIBRARIES=$PREFIX/python/lib/python$PYTHON_VER/config-$PYTHON_VER/libpython$PYTHON_VER.dll.a"
    fi
elif [ -n "$STAGE2" ]; then
...
cmake \
    ...
    -DLLVM_TOOLCHAIN_TOOLS="llvm-ar;llvm-ranlib;llvm-objdump;llvm-rc;llvm-cvtres;llvm-nm;llvm-strings;llvm-readobj;llvm-dlltool;llvm-pdbutil;llvm-objcopy;llvm-strip;llvm-cov;llvm-profdata;llvm-addr2line;llvm-symbolizer;llvm-windres;llvm-readelf" \
